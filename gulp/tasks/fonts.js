...
export const ttfToWoff = () => {
  return (
    app.gulp
      ...
      // Поиск фалов шрифтов .ttf
      .pipe(app.gulp.src(`${app.path.srcFolder}/assets/fonts/*.ttf`))
      // Конвертация в .woff2
      .pipe(ttf2woff2())
      // Выгрузка в папку с результатами
      .pipe(app.gulp.dest(`${app.path.build.fonts}`))
      // Поиск файлов шрифтов .woff и woff2
...
export const fontsStyle = () => {
  ...
  let fontsFile = `${app.path.buildFolder}/assets/fonts.css`;
  ...
  fs.readdir(app.path.build.fonts, function (err, fontsFiles) {
    if (fontsFiles) {
      ...
      if (!fs.existsSync(fontsFile)) {
        ...
        // Если файл есть - вывод сообщения
        console.log(
          'Файл "fonts.css" уже существует. Для обновления файла нужно его удалить!',
        );
      }
    }
  });
